...
def process_bronze_to_silver():
    """Main processing function. Returns True if data was processed, False otherwise."""
    ...
    # Filtra apenas registros válidos (com job_posting_id)
    # Isso permite processar partições que contêm mix de erros e dados válidos
    if "job_posting_id" in bronze_columns:
        df_valid = df_bronze.filter(col("job_posting_id").isNotNull())
        df_errors = df_bronze.filter(col("job_posting_id").isNull())
        error_count = df_errors.count()
        valid_count = df_valid.count()

        if error_count > 0:
            print(f"[bronze_to_silver] Found {error_count} error records (will be skipped)")

        if valid_count == 0:
            print("=" * 60)
            print("[bronze_to_silver] NENHUM REGISTRO VÁLIDO NA PARTIÇÃO")
            print("=" * 60)
            print(f"  Partição: year={YEAR}/month={MONTH.zfill(2)}/day={DAY.zfill(2)}/hour={HOUR.zfill(2)}")
            print(f"  Registros de erro: {error_count}")
            print(f"  Registros válidos: 0")
            print(f"  Ação: Job finalizado - nenhum dado a processar")
            print("=" * 60)
            return False

        # Substitui df_bronze pelos registros válidos
        df_bronze = df_valid
        print(f"[bronze_to_silver] Processing {valid_count} valid records")
    else:
        # Arquivo contém apenas estrutura de erro (sem coluna job_posting_id)
        print("=" * 60)
        print("[bronze_to_silver] CRAWL SEM RESULTADOS DETECTADO")
        print("=" * 60)

        if "error" in bronze_columns:
            if "error_code" in bronze_columns:
                df_bronze.select("timestamp", "error", "error_code").show(truncate=False)
            else:
                df_bronze.select("timestamp", "error").show(truncate=False)
        ...
        print(f"  Ação: Job finalizado - cenário esperado de negócio")
